...
(function (global) {
  ...
  const W = {
    ...
    Modal: { group: 'Layout', label: 'Modal', container: true,
      defaultProps: { title: 'Modal', open: false, width: '32rem', closeOnBackdrop: true, closeOnEscape: true, showCloseButton: true },
      fields: [{ key: 'title', label: 'Título', kind: 'text' }, { key: 'open', label: 'Aberto', kind: 'bool', bindable: true },
        { key: 'width', label: 'Largura', kind: 'text' }, { key: 'height', label: 'Altura', kind: 'text' },
        { key: 'closeOnBackdrop', label: 'Fechar ao clicar fora', kind: 'bool' }, { key: 'closeOnEscape', label: 'Fechar com Escape', kind: 'bool' },
        { key: 'showCloseButton', label: 'Mostrar botão de fechar', kind: 'bool' }, { key: 'onClose', label: 'onClose', kind: 'event' }, F.className],
      preview: (n, i) => `<div class="wk-card" style="min-width:280px;box-shadow:0 12px 28px rgba(15,23,42,.2)"><div style="font-weight:700;margin-bottom:12px">${showVal(p(n, 'title', 'Modal'))}</div>${i}</div>` },
