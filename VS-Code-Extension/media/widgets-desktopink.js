...
// The preview reuses the neutral WebInk canvas styles while the generated nodes
// map directly to DesktopInk's Swing widget vocabulary.
(function (global) {
  ...
  const W = {
    Window: { group: 'Layout', label: 'Window', container: true, defaultProps: {}, fields: [], preview: (n, i) => `<div class="wk-page">${i}</div>` },
    Container: { group: 'Layout', label: 'Container', container: true, defaultProps: {}, fields: [], preview: (n, i) => `<div class="wk-container">${i}</div>` },
    Section: { group: 'Layout', label: 'Section', container: true, defaultProps: {}, fields: [], preview: (n, i) => `<section class="wk-section">${i}</section>` },
    Grid: { group: 'Layout', label: 'Grid', container: true, defaultProps: { columns: 2, gap: 12 }, fields: [{ key: 'columns', label: 'Colunas', kind: 'number', min: 1, max: 12 }, { key: 'gap', label: 'Espaço entre filhos (gap)', kind: 'number', min: 0, max: 40 }], preview: (n, i) => `<div class="wk-grid" style="grid-template-columns:repeat(${p(n, 'columns', 2)},minmax(0,1fr))">${i}</div>` },
    Row: { group: 'Layout', label: 'Row', container: true, defaultProps: { gap: 8 }, fields: [{ key: 'gap', label: 'Espaço entre filhos (gap)', kind: 'number', min: 0, max: 40 }], preview: (n, i) => `<div class="wk-row">${i}</div>` },
    Col: { group: 'Layout', label: 'Col', container: true, defaultProps: {}, fields: [], preview: (n, i) => `<div class="wk-col">${i}</div>` },
    Card: { group: 'Layout', label: 'Card', container: true, defaultProps: {}, fields: [], preview: (n, i) => `<div class="wk-card">${i}</div>` },
    Modal: { group: 'Layout', label: 'Modal', container: true, defaultProps: { title: 'Modal' }, fields: [{ key: 'title', label: 'Título', kind: 'text' }], preview: (n, i) => `<div class="wk-card"><b>${show(p(n, 'title', 'Modal'))}</b>${i}</div>` },
    Divider: { group: 'Layout', label: 'Divider', container: false, defaultProps: {}, fields: [], preview: () => '<hr class="wk-divider">' },
    Spacer: { group: 'Layout', label: 'Spacer', container: false, defaultProps: {}, fields: [], preview: () => '<div class="wk-spacer"></div>' },
    Heading: { group: 'Tipografia', label: 'Heading', container: false, defaultProps: { level: 1, text: 'Título' }, fields: [{ key: 'level', label: 'Nível', kind: 'enum', options: [1, 2] }, F.text], preview: (n) => `<div class="wk-h wk-h${p(n, 'level', 1)}">${show(p(n, 'text', ''))}</div>` },
    Text: { group: 'Tipografia', label: 'Text', container: false, defaultProps: { text: 'Texto' }, fields: [F.text], preview: (n) => `<p class="wk-text">${show(p(n, 'text', ''))}</p>` },
    Badge: { group: 'Tipografia', label: 'Badge', container: false, defaultProps: { text: 'novo' }, fields: [F.text], preview: (n) => `<span class="wk-badge">${show(p(n, 'text', ''))}</span>` },
    ...
    Button: { group: 'Formulários', label: 'Button', container: false, defaultProps: { text: 'Botão' }, fields: [F.text, event], preview: (n) => `<button class="wk-btn wk-btn-primary">${show(p(n, 'text', ''))}</button>` },
    ...
    Table: { group: 'Dados', label: 'Table', container: false, defaultProps: { columns: ['Coluna'], rows: [['valor']] }, fields: [{ key: 'columns', label: 'Colunas (vírgula)', kind: 'csv' }, { key: 'rows', label: 'Linhas (uma por linha; células por vírgula)', kind: 'rows' }], preview: () => '<div class="wk-tablewrap">Tabela</div>' },
